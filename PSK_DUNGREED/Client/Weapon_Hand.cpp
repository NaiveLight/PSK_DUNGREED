...
int CWeapon_Hand::Update()
{
	...
	{
		//D3DXVec3Normalize(&m_tInfo.vDir, &m_tInfo.vDir);
		m_tInfo.vPos += m_tInfo.vDir * 500.f *TimeManager->GetDeltaTime();
	}
}
...
void CWeapon_Hand::UpdateMatrix()
{
	...
	D3DXMatrixTranslation(&matTrans, (m_bIsLeft? -m_tInfo.vPos.x : m_tInfo.vPos.x), m_tInfo.vPos.y, 0.f);
...
{
	m_bAttack = true;
	//CreateHitBox and Move Pos
}

void CWeapon_Hand::UpdateHitBox()
{
}
